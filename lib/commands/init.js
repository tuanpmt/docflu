...
/**
 * Initialize DocuFlu in current directory
 * Creates .env file with template configuration
 */
async function initProject() {
  const spinner = ora('Initializing DocuFlu...').start();
  ...
  try {
    ...
    const docusaurusConfig = path.join(projectRoot, 'docusaurus.config.ts');
    const docusaurusConfigJs = path.join(projectRoot, 'docusaurus.config.js');
    ...
    if (!isDocusaurus) {
      ...
      console.log(chalk.blue('💡 DocuFlu works best with Docusaurus projects that have:'));
...
`;
    ...
    spinner.succeed('DocuFlu initialized successfully');
    ...
    console.log(chalk.blue('   docuflu sync --docs --dry-run'));
    
    console.log(chalk.white('\n3. Perform your first sync:'));
    console.log(chalk.blue('   docuflu sync --docs'));
