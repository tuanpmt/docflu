...
async function syncFile(filePath, dryRun = false) {
  ...
  try {
    ...
    // Step 8: Process images and update page if needed
    ...
    // Update page with processed images if content changed
    if (contentWithImages !== parsedContent.htmlContent) {
      spinner.text = 'Updating page with images...';
  ...
  try {
    ...
    for (const [index, document] of documents.entries()) {
      ...
      try {
        ...
        const parser = new MarkdownParser();
        const parsedContent = await parser.parseMarkdown(document.content, document.frontmatter);
