...
const { migrateState, needsMigration } = require('./migrate-state');
...
class StateManager {
   ...
   */
  async init() {
    // Check if migration from .docflu/ is needed
    if (await needsMigration(this.projectRoot)) {
      await migrateState(this.projectRoot);
    }
  ...
  async createDefaultState() {
    ...
    console.log(chalk.green('✓ Created .docusaurus/sync-state.json'));
  }
  ...
  /**
   * Find page state by confluence ID
   */
  getPageStateByConfluenceId(confluenceId) {
    for (const [filePath, pageData] of Object.entries(this.state.pages)) {
      if (pageData.confluenceId === confluenceId) {
        return { filePath, ...pageData };
      }
    }
    return null;
  }
  ...
  /**
   * Find page state by title (for matching during pull)
   */
  getPageStateByTitle(title) {
    for (const [filePath, pageData] of Object.entries(this.state.pages)) {
