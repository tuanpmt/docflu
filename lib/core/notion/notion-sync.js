      ...
      if (block.type === 'paragraph' && block.paragraph.rich_text) {
        ...
        for (const textObj of richText) {
          ...
          for (const imageMatch of allMatches) {
            ...
            if (beforeText.trim()) {
              // Preserve original annotations when adding text before image
              const beforeTextObj = { text: { content: beforeText } };
              if (textObj.annotations) {
                beforeTextObj.annotations = textObj.annotations;
              }
              if (textObj.href) {
                beforeTextObj.href = textObj.href;
              }
              newRichText.push(beforeTextObj);
            }
          ...
          if (remainingText.trim()) {
            // Preserve original annotations when adding remaining text
            const remainingTextObj = { text: { content: remainingText } };
            if (textObj.annotations) {
              remainingTextObj.annotations = textObj.annotations;
            }
            if (textObj.href) {
              remainingTextObj.href = textObj.href;
            }
            newRichText.push(remainingTextObj);
