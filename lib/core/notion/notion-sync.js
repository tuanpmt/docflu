...
class NotionSync {
  ...
  async syncFile(filePath, options = {}) {
    try {
      ...
      // Process file with force option for single file sync
      const fileOptions = { ...options, force: true };
      const result = await this.processFile(relativePath, fileOptions);
      
      if (result.success) {
        // Single file sync never skips (always force)
        ...
        try {
          ...
          if (result.success) {
            if (result.dryRun) {
              ...
              console.log(chalk.gray(`⏭️ ${progress} ⚪ ${title} [SKIPPED]`));
            } else {
              this.syncStats.processed++;
              if (result.created) {
                this.syncStats.created++;
  ...
  async processFile(filePath, options = {}) {
    try {
      ...
      // Skip check only for non-force sync (docs sync with incremental mode)
      if (!options.force && !this.state.needsSync(filePath, currentHash)) {
      ...
      // For force sync, delete old page and remove from state
      if (options.force && this.state.getPageId(filePath)) {
        const oldPageId = this.state.getPageId(filePath);
        try {
          await this.client.updatePage(oldPageId, {
            archived: true
          });
        } catch (error) {
      ...
      // Use flat mode for single file sync, nested mode for docs sync
      const flatMode = options.singleFile || false;
      const parentPageId = await this.hierarchyManager.createPageHierarchy(filePath, this.config.rootPageId, flatMode);
      
      // Get or create content page (will always create new for force sync)
      const page = await this.hierarchyManager.getOrCreateContentPage(filePath, title, parentPageId);
      const isNewPage = !this.state.getPageId(filePath);
      ...
      // For non-force sync, clear existing content if updating
      if (!options.force && !isNewPage) {
        try {
          await this.clearPageContent(page.id);
        } catch (error) {
          if (error.message.includes('archived') || error.code === 'object_not_found') {
            // Page is archived or deleted, create a new one
            console.log(chalk.yellow(`⚠️ Page is archived/deleted, creating new page for: ${title}`));
            ...
            console.log(chalk.green(`✅ Created new page: ${title}`));
          } else {
            console.warn(chalk.yellow(`⚠️ Could not clear content, continuing with append: ${error.message}`));
          }
        }
      }
      ...
      // For force sync, always consider as "Created" since we create fresh page
      const action = (isNewPage || options.force) ? 'Created' : 'Updated';
      console.log(chalk.green(`  ✓ ${action}: ${title} (${totalBlocks} blocks)`));
      
      return { 
        success: true, 
        created: isNewPage || options.force, 
        updated: !isNewPage && !options.force,
        title,
        totalBlocks 
      };
  ...
  validateConfig() {
    ...
    // NOTION_ROOT_PAGE_ID is now optional - will auto-create if not provided
    if (this.config.rootPageId) {
      console.log(chalk.blue(`📄 Using provided root page: ${this.config.rootPageId}`));
    } else {
      console.log(chalk.blue(`🔄 Root page will be auto-created with title: ${this.config.rootTitle || 'Documentation'}`));
    }
