...
class MarkdownToBlocksConverter {
  ...
  splitIntoSections(markdown) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmed.startsWith('````') || trimmed.startsWith('```')) {
        if (inCodeBlock) {
          // Check if this is the matching closing backticks - must be exact match
          const currentBackticks = trimmed.match(/^`+/)[0];
          if (currentBackticks === codeBlockType) {
            // End of code block
          ...
          inCodeBlock = true;
          codeBlockType = trimmed.match(/^`+/)[0]; // Store exact backtick count
          currentSection.push(line);
          continue;
        }
      }
  ...
  parseRichText(text) {
    ...
    // 4. Bold+Italic (triple markers - highest priority to avoid conflicts)
    const boldItalicMatches = [];
    let boldItalicMatch;
    const boldItalicRegex = /(\*\*\*|___)([^*_]+)\1/g;
    while ((boldItalicMatch = boldItalicRegex.exec(text)) !== null) {
      // Skip if inside image, link or code
      const isInsideOther = [...imageMatches, ...linkMatches, ...codeMatches].some(other => 
        boldItalicMatch.index >= other.start && boldItalicMatch.index < other.end
      );
      if (!isInsideOther) {
        boldItalicMatches.push({
          ...
          full: boldItalicMatch[0]
        });
      }
    }

    // 5. Bold (handle both ** and __ patterns)
    const boldMatches = [];
    let boldMatch;
    const boldRegex = /(\*\*|__)([^*_]+)\1/g;
    while ((boldMatch = boldRegex.exec(text)) !== null) {
      // Skip if inside image, link, code, or bold+italic
      const isInsideOther = [...imageMatches, ...linkMatches, ...codeMatches, ...boldItalicMatches].some(other => 
    ...
    // 6. Italic (handle both * and _ patterns, but not inside bold or bold+italic)
    const italicMatches = [];
    let asteriskItalicMatch, underscoreItalicMatch;
    
    // Process * italic - manually find all asterisk pairs to avoid regex greedy issues
    ...
    // Find valid single asterisk pairs (not part of ** or ***)
    for (let i = 0; i < asteriskPositions.length - 1; i++) {
      const start = asteriskPositions[i];
      const end = asteriskPositions[i + 1];
      
      // Check if this is a single asterisk pair (not ** or ***)
      const isStartSingle = (start === 0 || text[start - 1] !== '*') && (start + 1 >= text.length || text[start + 1] !== '*');
      const isEndSingle = (end === 0 || text[end - 1] !== '*') && (end + 1 >= text.length || text[end + 1] !== '*');
      
      if (isStartSingle && isEndSingle) {
        const content = text.substring(start + 1, end);
        
        // Skip if inside image, link, code, bold+italic, or bold
        const isInsideOther = [...imageMatches, ...linkMatches, ...codeMatches, ...boldItalicMatches, ...boldMatches].some(other => 
          start >= other.start && start < other.end
        );
        
        if (!isInsideOther && content.trim() && !content.includes('*')) {
          italicMatches.push({
            type: 'italic',
            start: start,
            end: end + 1,
    ...
    // Process _ italic - manually find all underscore pairs to avoid regex greedy issues
    ...
    // Find valid single underscore pairs (not part of __ or ___)
    for (let i = 0; i < underscorePositions.length - 1; i++) {
      const start = underscorePositions[i];
      const end = underscorePositions[i + 1];
      
      // Check if this is a single underscore pair (not __ or ___)
      const isStartSingle = (start === 0 || text[start - 1] !== '_') && (start + 1 >= text.length || text[start + 1] !== '_');
      const isEndSingle = (end === 0 || text[end - 1] !== '_') && (end + 1 >= text.length || text[end + 1] !== '_');
      ...
      if (isStartSingle && isEndSingle) {
        ...
        if (!isInsideOther && content.trim() && !content.includes('_')) {
          italicMatches.push({
            type: 'italic',
            start: start,
            end: end + 1,
    ...
    while ((strikeMatch = strikeRegex.exec(text)) !== null) {
      ...
      const isInsideOther = [...imageMatches, ...linkMatches, ...codeMatches, ...boldItalicMatches, ...boldMatches, ...italicMatches].some(other => 
    ...
    const allMatches = [...imageMatches, ...linkMatches, ...codeMatches, ...boldItalicMatches, ...boldMatches, ...italicMatches, ...strikeMatches];
    ...
    for (const match of allMatches) {
      ...
      switch (match.type) {
        ...
        case 'bold':
          ...
          break;
        case 'bold_italic':
          textObj.annotations = { bold: true, italic: true };
          break;
        case 'bold_link':
