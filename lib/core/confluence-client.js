...
class ConfluenceClient {
  ...
  async getPageAttachments(pageId) {
    try {
      ...
      console.warn(chalk.yellow(`⚠️ Could not get attachments for page ${pageId}: ${error.message}`));
      return [];
    }
  }
  ...
  /**
   * Download attachment by attachment object
   * @param {Object} attachment - Attachment object from getPageAttachments
   * @returns {Buffer} - Attachment data
   */
  async downloadAttachmentById(attachment) {
    try {
      const downloadUrl = `${this.config.baseUrl}/wiki/${attachment._links.download}`;
      ...
      throw new Error(`Failed to download attachment ${attachment.title}: ${error.response?.data?.message || error.message}`);
    }
  }
