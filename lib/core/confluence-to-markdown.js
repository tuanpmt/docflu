...
class ConfluenceToMarkdown {
  ...
  async processAttachments($, pageId, relativePath) {
    const attachments = [];
    const attachmentElements = $('ri\\:attachment, attachment');
    
    for (let i = 0; i < attachmentElements.length; i++) {
      const element = attachmentElements[i];
      const filename = $(element).attr('ri:filename') || $(element).attr('filename');
      
      if (filename) {
        try {
          // Download attachment from Confluence
          const attachmentData = await this.confluenceClient.downloadAttachment(pageId, filename);
          
          // Determine local path for the attachment
          const attachmentDir = path.dirname(path.join(this.projectRoot, relativePath));
          ...
          console.warn(`Warning: Could not download attachment ${filename}: ${error.message}`);
        }
      }
    }
