...
class GDocsDiagramProcessor {
  ...
  async processAllDiagrams(markdownContent) {
    ...
    try {
      ...
      // Replace diagrams in markdown with placeholders for native image insertion
      // Only replace diagrams that were successfully processed
      let diagramIndex = 0;
      for (const [originalContent, replacement] of diagramMap) {
        const placeholder = `[DIAGRAM_PLACEHOLDER${diagramIndex}]`;
        processedMarkdown = processedMarkdown.replace(originalContent, placeholder);
        
        // Update the replacement object to include placeholder info
