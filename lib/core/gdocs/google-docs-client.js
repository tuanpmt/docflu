...
class GoogleDocsClient {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        server.on('request', async (req, res) => {
          try {
            const parsedUrl = url.parse(req.url, true);
              ...
              // Close server with callback to ensure it's fully closed
              server.close(() => {
                console.log(chalk.gray('🔌 OAuth server closed'));
              });
            }
          } catch (error) {
            console.error(chalk.red('❌ Error handling callback:'), error.message);
            server.close(() => {
              console.log(chalk.gray('🔌 OAuth server closed (error case)'));
            });
            reject(error);
          }
        });
        ...
        server.on('error', (error) => {
          ...
          server.close(() => {
            console.log(chalk.gray('🔌 OAuth server closed (server error)'));
          });
          reject(error);
        });
  ...
  /**
   * Clear saved tokens (for permission errors)
   */
  async clearTokens() {
    try {
      if (await fs.pathExists(this.tokensPath)) {
        await fs.remove(this.tokensPath);
        console.log(chalk.yellow('🗑️ Cleared existing Google OAuth2 tokens'));
      }
      ...
      console.warn(chalk.yellow('⚠️ Could not clear tokens:', error.message));
    }
  }
  ...
  async cleanup() {
    try {
      // Clear OAuth2 client credentials to close any connections
      if (this.oauth2Client) {
        this.oauth2Client.setCredentials({});
      }
      
      // console.log(chalk.gray('🧹 Google Docs client cleanup completed'));
      
    } catch (error) {
      console.warn(chalk.yellow('⚠️ Google Docs client cleanup warning:', error.message));
    }
  }
}
