...
class GoogleDocsConverter {
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (trimmedLine === '') {
          ...
          // Code block - check if it's a diagram type that should be skipped
          const codeBlockResult = this.extractCodeBlock(lines, i);
          if (codeBlockResult) {
            const { content, language, endIndex } = codeBlockResult;
            
            // Skip diagram code blocks that should have been processed by image processor
            // These should have been converted to placeholders already
            const diagramLanguages = ['mermaid', 'plantuml', 'dot', 'graphviz', 'd2'];
            if (diagramLanguages.includes(language?.toLowerCase())) {
              console.log(chalk.yellow(`⚠️ Skipping unprocessed ${language} diagram code block - likely missing CLI tool`));
              
              debugInfo.processing.elements.push({
                lineNumber: i + 1,
                type: 'skipped_diagram',
                language: language,
                content: content.substring(0, 100) + (content.length > 100 ? '...' : ''),
                linesSpanned: endIndex - i + 1,
                reason: 'Diagram should have been processed by image processor but was not - skipping to avoid duplication'
              });
              
              i = endIndex; // Skip these lines completely
              continue;
            }
            
            // Regular code block - insert language label if present
            let totalText = '';
            if (language) {
          ...
          // Lists - extract and process inline formatting for each item
          ...
          items.forEach((item, index) => {
            ...
            // Detect inline formatting in the item text
            const inlineFormats = this.detectInlineFormatting(item);
            
            // Add prefix + processed text
            listText += prefix + inlineFormats.processedText + '\n';
            
            // Adjust formatting positions to account for prefix and previous items
            if (inlineFormats.formats && inlineFormats.formats.length > 0) {
              inlineFormats.formats.forEach(format => {
                listFormats.push({
                  start: currentOffset + prefixLength + format.start,
                  end: currentOffset + prefixLength + format.end,
                  style: format.style
                });
              });
            }
            
            // Update offset for next item (prefix + text + newline)
            currentOffset += prefixLength + inlineFormats.processedText.length + 1;
          });
          ...
          // Store formatting for step 2 if any items have formatting
          if (listFormats.length > 0) {
            formattingForStep2.push({
              type: 'list',
              originalText: listText.trim(),
              processedText: listText.trim(),
              formats: listFormats,
              requestIndex: requests.length - 1
            });
          }
          ...
          debugInfo.processing.elements.push({
            ...
            items: items,
            linesSpanned: endIndex - i + 1,
            needsFormatting: listFormats.length > 0,
            inlineFormats: listFormats.length
          });
          ...
          if (tableResult) {
            ...
            if (tableData) {
              ...
              continue;
                      ...
                      // Handle image placeholders - convert to text then replace with native images
            if (trimmedLine.match(/^\[(DIAGRAM_PLACEHOLDER|IMAGE_PLACEHOLDER)\d+\]$/)) {
              const placeholderText = trimmedLine + '\n';
              
              // Add placeholder as regular text
              requests.push({
                insertText: {
  ...
  detectInlineFormatting(text) {
    ...
    // Find **bold**, *italic*, ***bold+italic***, and `code` patterns
    const allMatches = [];
    
    // Find ***bold+italic*** and ___bold+italic___ patterns (highest priority)
    let match;
    ...
    while ((match = boldItalicRegex.exec(text)) !== null) {
      allMatches.push({
        ...
        priority: 0 // Highest priority to avoid conflicts
      });
    }
    ...
    // Find *italic* and _italic_ patterns (but not inside bold or bold+italic)
    const italicRegex = /(\*|_)(.*?)\1/g;
    while ((match = italicRegex.exec(text)) !== null) {
      // Skip if this is part of a bold or bold+italic pattern
      ...
      if (!isBoldMarker && !isBoldItalicMarker) {
        allMatches.push({
          start: match.index,
          end: match.index + match[0].length,
          content: match[2],
