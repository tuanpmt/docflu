...
class ImageProcessor {
  ...
  extractImages(markdownContent, filePath) {
    ...
    while ((match = markdownImageRegex.exec(markdownContent)) !== null) {
      ...
      // Clean src URL by removing any whitespace or extra content
      ...
      const imageInfo = {
        ...
        index: match.index,
        type: 'markdown',
        isLocal: this.isLocalImage(cleanSrc),
        absolutePath: null
      };

      // Resolve absolute path cho local images
      if (imageInfo.isLocal) {
        imageInfo.absolutePath = this.resolveImagePath(cleanSrc, filePath);
      }
  ...
  convertImagesToGoogleDriveFormat(markdownContent, attachmentMap) {
    ...
    processedContent = processedContent.replace(/!\[(.*?)\]\(([^)]+?)(?:\s+"([^"]*)")?\)/g, (match, alt, src, title) => {
      // Clean src URL by removing any whitespace or extra content
      const cleanSrc = src.trim().split(/\s+/)[0];
      const attachment = attachmentMap.get(cleanSrc);
      
      if (attachment) {
        // Image uploaded to Google Drive - use simple format without title to avoid parsing issues
        const newAlt = alt || attachment.alt || 'Image';
        return `![${newAlt}](${attachment.url})`;
      } else if (this.isRemoteUrl(cleanSrc)) {
        // External image that wasn't processed - keep as is
        return match;
      } else {
        // Local image that failed to upload - show warning
        console.warn(chalk.yellow(`⚠️ Image not uploaded: ${cleanSrc}`));
        return `![${alt || 'Image not found'}](${cleanSrc})`;
      }
    });
    ...
    processedContent = processedContent.replace(/<img[^>]+src=["']([^"']+)["'][^>]*>/gi, (match, src) => {
      ...
      if (attachment) {
        ...
        // Local image that failed to upload - show warning
        console.warn(chalk.yellow(`⚠️ HTML image not uploaded: ${cleanSrc}`));
