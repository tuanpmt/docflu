...
class GoogleDocsSync {
  ...
  async syncDocumentAppend(document, rootDocumentId) {
    ...
    try {
      ...
      if (formattingForStep2 && formattingForStep2.length > 0) {
        ...
        // CRITICAL: For append mode, reset only NEW content to avoid affecting previous content
        ...
        if (allFormattingRequests.length > 0) {
          ...
          // Debug: Log formatting requests to file
          await this.debugger.logRequestBatch(allFormattingRequests, 'formatting-append', {
            resetRequests: resetColorRequests.length,
            specificRequests: formattingRequests.length,
            contentStartIndex: contentStartIndex
          });
    ...
    try {
      ...
      if (formattingForStep2 && formattingForStep2.length > 0) {
        ...
        if (allFormattingRequests.length > 0) {
          ...
          // Debug: Log formatting requests to file
          await this.debugger.logRequestBatch(allFormattingRequests, 'formatting-replace', {
            resetRequests: resetColorRequests.length,
            specificRequests: formattingRequests.length
          });
  ...
  createTableCellGistRequests(tableData, tableStartIndex) {
    ...
    // Debug: Calculated indices logged to file with request batch
  ...
  async processImageInsertionBatches(imageRequests, documentId) {
    ...
    try {
      ...
      console.log(chalk.gray(`📦 Processing ${totalBatches} batches of image requests...`));
      ...
      for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
        ...
        if (batchRequests.length > 0) {
          ...
          console.log(chalk.gray(`   ✅ Batch ${batchIndex + 1}/${totalBatches}: ${imagesInBatch} images inserted`));
        }
      }
      
      console.log(chalk.green(`✅ Successfully inserted ${totalInserted} native images in ${totalBatches} batches`));
      return { success: true, insertedCount: totalInserted };
      
    } catch (error) {
      console.warn(chalk.yellow(`⚠️ Batch image insertion failed: ${error.message}`));
  ...
  async insertImagesWithBatchProcessing(imageRequests, documentId) {
    ...
    console.warn(chalk.yellow(`⚠️ Native image insertion failed: ${batchResult.error}`));
    ...
    try {
      ...
      for (const imageRequest of imageRequests) {
        ...
        if (placeholderPosition) {
          ...
          console.warn(chalk.yellow(`⚠️ Fallback: Placeholder not found: ${placeholder}`));
        }
      }
      ...
      for (const item of placeholderPositions) {
        ...
        console.log(chalk.gray(`   📝 Fallback markdown: ${imageRequest.altText} at index ${position.startIndex}`));
      }
      ...
      if (fallbackRequests.length > 0) {
        ...
        console.log(chalk.green(`✅ Fallback: Inserted ${placeholderPositions.length} markdown images`));
      }
      
    } catch (error) {
      console.error(chalk.red(`❌ Fallback insertion failed: ${error.message}`));
      throw error;
    }
  }
  ...
  async createImageInsertionRequests(imageRequests, document) {
    ...
    try {
      ...
      if (this.debug) {
        ...
        const placeholderMatches = fullText.match(/\[(IMAGE_PLACEHOLDER|DIAGRAM_PLACEHOLDER)\d+\]/g);
      ...
      for (let i = 0; i < imageRequests.length; i++) {
        ...
        if (placeholderPosition) {
          ...
          if (this.debug) {
            ...
            const availablePlaceholders = fullText.match(/\[(IMAGE_PLACEHOLDER|DIAGRAM_PLACEHOLDER)\d+\]/g);
      ...
      for (const item of placeholderPositions) {
        ...
        if (this.debug) {
          ...
          console.log(chalk.gray(`🐛 URL length: ${imageRequest.url.length}`));
          console.log(chalk.gray(`🐛 URL valid: ${this.isValidImageUrl(imageRequest.url)}`));
        }
      }
  ...
  /**
   * Validate if URL is a valid image URL for Google Docs
   */
  isValidImageUrl(url) {
    try {
      ...
      // Check URL length (Google Docs has limits)
      if (url.length > 2048) {
        return false;
      }
