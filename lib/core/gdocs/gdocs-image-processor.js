...
class GDocsImageProcessor {
  ...
  async processImages(markdown, filePath) {
    ...
    try {
      ...
      if (this.diagramProcessor) {
        ...
        // Convert diagram map to image requests - diagrams already have placeholders from diagram processor
        for (const [originalContent, replacement] of diagramResult.diagramMap) {
          // Add to native image requests using the placeholder created by diagram processor
          imageRequests.push({
            type: 'diagram',
            url: replacement.url,
            ...
            placeholder: replacement.placeholder // Use placeholder from diagram processor
          });
        }
  ...
  async processRegularImagesAsNative(markdown, filePath, debugInfo = null) {
    try {
      ...
      // Process markdown images: ![alt](src) or ![alt](src "title")
      ...
      while ((match = markdownImagePattern.exec(result.processedMarkdown)) !== null) {
        ...
        // Clean URL by removing any title text and whitespace
        ...
        if (cleanUrl.includes('drive.google.com') || cleanUrl.includes('googleusercontent.com')) {
          const placeholder = `[IMAGE_PLACEHOLDER${imageIndex}]`;
          imageIndex++;
      ...
      while ((match = htmlImagePattern.exec(processedMarkdown)) !== null) {
        ...
        // Clean URL by removing any query parameters or title text
        const cleanUrl = url.trim().split(/\s+/)[0];
  ...
  async saveDebugInfo(debugInfo, suffix = '') {
    ...
    try {
      ...
      const summary = {
        ...
        inputLength: debugInfo.processing.phases.length > 0 ? debugInfo.processing.phases[0].inputLength : 0,
        filePath: debugInfo.filePath,
        phases: debugInfo.processing.phases.length,
        imagesFound: debugInfo.processing.images.length,
        diagramsFound: debugInfo.processing.diagrams.reduce((sum, d) => sum + (d.count || 0), 0),
        uploadsCompleted: debugInfo.processing.uploads ? debugInfo.processing.uploads.length : 0,
  ...
  async cleanup() {
    try {
      ...
      if (this.driveClient) {
        // Drive client has its own cleanup
      }
      

      
    } catch (error) {
      console.warn(chalk.yellow('⚠️ Google Docs image processor cleanup warning:', error.message));
    }
  }
}
