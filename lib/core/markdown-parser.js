...
class MarkdownParser {
  ...
  replaceDiagramCodeBlocksInConfluence(confluenceContent, diagramMap) {
    ...
    // Define diagram types and their patterns
    const diagramTypes = {
      mermaid: /```mermaid\n([\s\S]*?)\n```/g,
      plantuml: /```plantuml\n([\s\S]*?)\n```/g,
    ...
    // Also handle Confluence code blocks
    const confluenceCodePatterns = {
      mermaid: /<ac:structured-macro ac:name="code"[^>]*>\s*<ac:parameter ac:name="language">mermaid<\/ac:parameter>\s*<ac:plain-text-body><!\[CDATA\[([\s\S]*?)\]\]><\/ac:plain-text-body>\s*<\/ac:structured-macro>/g,
      plantuml: /<ac:structured-macro ac:name="code"[^>]*>\s*<ac:parameter ac:name="language">plantuml<\/ac:parameter>\s*<ac:plain-text-body><!\[CDATA\[([\s\S]*?)\]\]><\/ac:plain-text-body>\s*<\/ac:structured-macro>/g,
      graphviz: /<ac:structured-macro ac:name="code"[^>]*>\s*<ac:parameter ac:name="language">(?:dot|graphviz)<\/ac:parameter>\s*<ac:plain-text-body><!\[CDATA\[([\s\S]*?)\]\]><\/ac:plain-text-body>\s*<\/ac:structured-macro>/g,
      d2: /<ac:structured-macro ac:name="code"[^>]*>\s*<ac:parameter ac:name="language">d2<\/ac:parameter>\s*<ac:plain-text-body><!\[CDATA\[([\s\S]*?)\]\]><\/ac:plain-text-body>\s*<\/ac:structured-macro>/g
    };
    ...
    for (const [type, regex] of Object.entries(diagramTypes)) {
      processedContent = processedContent.replace(regex, (match, diagramCode) => {
        ...
        const diagramId = this.generateDiagramId(type, cleanCode);
