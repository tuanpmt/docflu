...
class MarkdownParser {
  constructor(projectRoot = null, stateManager = null, confluenceClient = null) {
    ...
    // Add custom fence renderer to handle title attribute
    this.md.renderer.rules.fence = (tokens, idx, options, env, slf) => {
      ...
      if (info) {
        // Parse info string for language and title
        // Format: jsx title="filename.js" or jsx title='filename.js'
      ...
      // Simple HTML escape function for attributes only
      ...
      // For code content, only escape essential HTML characters that would break structure
      const escapeCodeContent = (str) => {
        return str
          .replace(/&/g, '&amp;')
          .replace(/</g, '&lt;')
          .replace(/>/g, '&gt;');
      ...
      if (options.highlight) {
        const highlighted = options.highlight(token.content, langName) || escapeCodeContent(token.content);
        if (highlighted !== token.content) {
          return `<pre><code class="language-${langName}"${title ? ` data-title="${escapeAttribute(title)}"` : ''}>${highlighted}</code></pre>\n`;
        }
      }

      return `<pre><code class="language-${langName}"${title ? ` data-title="${escapeAttribute(title)}"` : ''}>${escapeCodeContent(token.content)}</code></pre>\n`;
    };
  ...
  async parseMarkdown(markdown, frontmatter = {}, currentFilePath = null, baseUrl = null, pageId = null) {
    ...
    // Process Mermaid diagrams AFTER HTML conversion if mermaid processor is available and pageId provided
    if (this.mermaidProcessor && pageId) {
      ...
      // Replace Mermaid code blocks in the final Confluence content
      confluenceContent = this.mermaidProcessor.replaceMermaidInConfluenceContent(confluenceContent, mermaidResult.diagramMap);
  ...
  convertToConfluenceFormat(html) {
    ...
    // Convert code blocks with title support
    confluenceXML = confluenceXML.replace(
      /<pre><code class="language-(\w+)"[^>]*data-title="([^"]*)"[^>]*>(.*?)<\/code><\/pre>/gs,
      (match, lang, title, code) => {
    ...
    // Convert code blocks without title
    confluenceXML = confluenceXML.replace(
