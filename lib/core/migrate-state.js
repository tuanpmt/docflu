...
/**
 * Migrate state from .docflu/ to .docusaurus/ directory
 * @param {string} projectRoot - Project root directory
 */
async function migrateState(projectRoot) {
  const oldStateDir = path.join(projectRoot, '.docflu');
  ...
  try {
    ...
    if (!await fs.pathExists(oldStateFile)) {
      console.log(chalk.blue('ℹ️ No .docflu/sync-state.json found, no migration needed'));
      return false;
    }
    ...
    console.log(chalk.cyan('🔄 Migrating state from .docflu/ to .docusaurus/...'));
    ...
    const backupDir = path.join(projectRoot, '.docflu.backup');
    await fs.move(oldStateDir, backupDir);
    console.log(chalk.blue(`📦 Backed up old state to .docflu.backup/`));

    console.log(chalk.green('🎉 Migration completed successfully!'));
    console.log(chalk.white('   State is now stored in .docusaurus/sync-state.json'));
    console.log(chalk.gray('   You can safely delete .docflu.backup/ after verifying everything works'));
...
async function needsMigration(projectRoot) {
  const oldStateFile = path.join(projectRoot, '.docflu', 'sync-state.json');
