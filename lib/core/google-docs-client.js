...
class GoogleDocsClient {
  ...
  /**
   * Add dummy content to a document for testing
   */
  async addDummyContent(documentId) {
    try {
      console.log(chalk.blue(`📝 Adding dummy content to document ${documentId}`));
      ...
      const requests = [
        {
          insertText: {
            ...
            text: 'Welcome to docflu Google Docs Integration!\n\n',
          },
        },
        {
          updateTextStyle: {
        ...
        },
        {
          insertText: {
            ...
            text: 'This is a test document created by docflu CLI tool.\n\n' +
                  'Features:\n' +
                  ...
                  '2. Add tab hierarchy support\n' +
                  '3. Convert internal references\n' +
                  '4. Process images and diagrams\n\n' +
      ...
      console.log(chalk.green('✅ Dummy content added successfully!'));
      console.log(chalk.gray(`📊 Applied ${requests.length} formatting requests`));
      
      return response.data;
    } catch (error) {
      console.error(chalk.red('❌ Failed to add dummy content:'), error.message);
      throw error;
    }
  }
  ...
  async testConnection() {
    try {
      ...
      const testDoc = await this.createDocument('docflu API Test');
      
      // Add some content
      await this.addDummyContent(testDoc.documentId);
      
      console.log(chalk.green('✅ Google Docs API connection successful!'));
      console.log(chalk.cyan(`🔗 Test document: ${testDoc.url}`));
