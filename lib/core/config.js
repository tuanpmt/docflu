  ...
  buildNotionConfig(env) {
    ...
    if (missing.length > 0) {
      throw new Error(
        ...
        `For Internal integrations, you must manually create a root page:\n` +
        `1. Create a page in your Notion workspace\n` +
        `2. Share it with your DocFlu integration\n` +
        `3. Copy the page ID from the URL\n` +
        `4. Set NOTION_ROOT_PAGE_ID in your .env file`
      );
    }

    return {
      apiToken: env.NOTION_API_TOKEN,
      rootPageId: env.NOTION_ROOT_PAGE_ID, // Required for Internal integrations
      // Optional settings
      excludePatterns: env.DOCFLU_EXCLUDE_PATTERNS ? env.DOCFLU_EXCLUDE_PATTERNS.split(',') : [],
      retryCount: parseInt(env.DOCFLU_RETRY_COUNT) || 3
    };
