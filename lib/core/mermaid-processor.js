...
class MermaidProcessor {
  ...
  async generateDiagramImage(diagram) {
    ...
    try {
      ...
      if (await fs.pathExists(outputFile)) {
        console.log(chalk.green(`✅ Generated diagram image: ${diagram.id}.svg`));
        return outputFile;
      } else {
      ...
      console.warn(chalk.yellow(`⚠️ Failed to generate diagram ${diagram.id}: ${error.message}`));
      return null;
    } finally {
      // Clean up input file
  ...
  async uploadDiagramImage(pageId, imagePath, diagramId) {
    ...
    try {
      console.log(chalk.blue(`📤 Uploading diagram: ${fileName}`));
      ...
      form.append('comment', `Mermaid diagram generated by DocFlu CLI`);
      ...
      console.log(chalk.green(`✅ Uploaded diagram: ${fileName}`));
      return attachmentInfo;

    } catch (error) {
      console.warn(chalk.yellow(`⚠️ Failed to upload diagram ${fileName}: ${error.message}`));
...
<p><em>Mermaid Diagram</em></p>`;
        ...
        console.warn(chalk.yellow(`⚠️ Diagram not processed: ${diagramId}`));
        return `<ac:structured-macro ac:name="code">
          <ac:parameter ac:name="language">mermaid</ac:parameter>
          <ac:plain-text-body><![CDATA[${diagramCode.trim()}]]></ac:plain-text-body>
        </ac:structured-macro>

<p><em>⚠️ Mermaid diagram could not be converted to image</em></p>`;
      }
    });
  }
...
<p><em>⚠️ Mermaid diagram could not be converted to image</em></p>`;
  ...
  async processMermaidDiagrams(pageId, markdownContent) {
    ...
    console.log(chalk.blue(`🎨 Found ${diagrams.length} Mermaid diagram(s) to process`));
    ...
    for (const diagram of diagrams) {
      try {
        ...
        console.warn(chalk.yellow(`⚠️ Error processing diagram ${diagram.id}: ${error.message}`));
        failedCount++;
      }
    }
    ...
    if (processedCount > 0) {
      console.log(chalk.green(`🎨 Processed ${processedCount}/${diagrams.length} Mermaid diagrams`));
    }
  ...
  async cleanup() {
    try {
      if (await fs.pathExists(this.tempDir)) {
        ...
        console.log(chalk.gray('🧹 Cleaned up temp directory'));
      }
    } catch (error) {
